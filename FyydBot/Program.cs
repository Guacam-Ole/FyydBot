...
{
    private static bool cancellationReceived = false;

    public static async Task Main(string[] args)
    {
        AssemblyLoadContext.Default.Unloading += UnloadSigTerm;
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (notifications.Count != 0)
                        logger.LogInformation("Found '{count}' unread notifications", notifications.Count);
                }
                catch (Exception e)
                {
                ...
                foreach (var notification in notifications)
                {
                    ...
                    if (searchContents == null)
                    {
                        ...
                        await mastodon.SendErrorResponse(notification.Status, replyId,
                            "Sorry. Ich versteh das nicht wirklich. Bin ja auch nur ein dummer Bot. \ud83e\udd16 Wenn Du mir gar keine Frage stellen wolltest. Ignoriere das einfach :)");
                        await mastodon.DismissNotification(notification.Id);
                        continue;
                    }

                    if (string.IsNullOrWhiteSpace(searchContents.PodcastName) &&
                        string.IsNullOrWhiteSpace(searchContents.Keywords))
                    {
                        logger!.LogWarning("Succesfully parsed search query, but no keywords found: '{content}'",
                            notification.Status.Content);
                        await mastodon.SendErrorResponse(notification.Status, replyId,
                            "Ich konnte weder einen Podcast, noch einen Suchbegriff aus Deiner Anfrage bestimmen.\u2753\ufe0f\nBeispiel für eine Anfrage, die ich verstehe:'Ich suche einen Podcast über Gummibärchen'");
                        continue;
                    }
                    ...
                    {
                        logger!.LogInformation("Succesfully parsed search query, but no matchs from Fyyd: '{content}'",
                            notification.Status.Content);
                        await mastodon.SendErrorResponse(notification.Status, replyId,
                            "Ich habe Dich zwar verstanden, aber keine passenden Podcasts gefunden. \ud83e\udd37");
                        continue;
                    }

                    await mastodon.CreateResponseFromSearchResult(notification.Status, replyId, searchContents,
                        searchResponse);
                }
    ...
    private static void UnloadSigTerm(AssemblyLoadContext obj)
    {
